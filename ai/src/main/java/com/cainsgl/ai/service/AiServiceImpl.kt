...
class AiServiceImpl : AiService
{
    ...
    companion object
    {
        ...
        val GENERATE_TAG_PROMPT = """
            ...
            4. 所有标签的分数总和必须等于1.0。
            5. 生成的标签必须是大类，不能具体，如算法，生活，Java，编程等。
        ...
        val GENERATE_SUMMARY_PROMPT = """
            ...
            内容处理规则：
            1. 仅基于用户后续提供的文本进行分析，不推断、不添加文本中未包含的任何信息。
            2. 无需处理任何格式符号，直接以用户提供的Markdown或纯文本为分析依据。
            摘要生成标准：
            1. 摘要应精炼概括原文核心内容、主要观点及关键信息。
            2. 保持原文主旨不变，不引入个人评论或外部知识。
            3. 摘要长度应适中，不得超过250字，力求清晰、连贯、完整。
            4. 生成的摘要只能是纯文本，不能是markdown
        """.trimIndent()
        val DEFAULT_PROMPT = """
    ...
    fun getSummaryByContent(content: String): Any
    {
        val res = arkService.use()
