...
import com.cainsgl.article.repository.PostDocumentRepository
...
class PostDocumentService {
    @Resource
    lateinit var postDocumentRepository: PostDocumentRepository
    ...
    fun search(
        ...
        if (useTag) {
            shouldQueries.add(
                Query.Builder()
                    .terms(TermsQuery.Builder()
                        .field("tags")
                        .terms { t -> t.value(listOf(FieldValue.of(query))) }
                        .boost(2.5f)
                        .build())
                    .build()
            )
        ...
        val sourceFilter = FetchSourceFilterBuilder()
            .withIncludes("id", "title", "img", "tags", "score","summary")
            .build()
        ...
        val nativeQueryBuilder = NativeQuery.builder()
            ...
            .withHighlightQuery(HighlightQuery(highlight, PostDocument::class.java))
            .withMaxResults(size)
