...
class FileController
{
    ...
    fun uploadFile(@RequestParam("file") file: MultipartFile): FileUrlEntity
    {
        ...
        if (userService.mallocMemory(userId,file.size.toInt()))
        {
            ...
            val fileUrlEntity = FileUrlEntity(userId = userId, url = fileUrl,name=file.originalFilename, fileSize = file.size.toInt())
    ...
    @SaIgnore
    @GetMapping
    fun getFile(@RequestParam("f") shorUrl: Long, @RequestParam("width", required = false) width: String?, request:HttpServletRequest,response: HttpServletResponse):Any?
    {
        if(width!=null)
        {
