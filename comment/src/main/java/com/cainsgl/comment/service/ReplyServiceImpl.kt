...
class ReplyServiceImpl : ServiceImpl<ReplyMapper, ReplyEntity>() {
    ...
    private lateinit var replyMapper: ReplyMapper
     ...
     fun getByParCommentIdCursor(parCommentId: Long, lastCreatedAt: LocalDateTime?, lastLikeCount: Int?,lastId:Long?): List<ReplyEntity> {
        return replyMapper.selectByParCommentIdCursor(parCommentId, lastCreatedAt, lastLikeCount,lastId)
    }

     fun getByPostCommentIdCursor(postCommentId: Long, lastCreatedAt: LocalDateTime?, lastLikeCount: Int?,lastId:Long?): List<ReplyEntity> {
